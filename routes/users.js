module.exports = app => {
  ...
  app.route('/user')
     ...
     *    HTPP/1.1 412 Precondition Failed
     */
    .get(async (req, res) => {
      try {
        const { id } = req.user;
    ...
    /**
     * @api {delete} /user Exclui usuário autentcado
     * @apiGroup Usuário
     * @apiHeader {String} Authorization Token de usuário
     *@apiHeaderExample {json} Header
     *   {"Authorization": "JWt xyz.abc.123.hgf"}
    ...
    .delete(async (req, res) => {
      try {
        ...
        res.status(412).json({ mgs: err.message });
      }
   ...
   * @apiSuccess {String} name Nome
   * @apiSuccess {String} email Email
   * @apiSuccess {String} password Senha crptografada
